...
tidepool3.load_level = function()
	...
	define_tile_code("bubble_down")
	local bubble_xy_2 = {}
	level_state.callbacks[#level_state.callbacks+1] = set_pre_tile_code_callback(function(x, y, layer)
		bubble_xy_2[#bubble_xy_2 + 1] = {x,y}
		return true
	end, "bubble_down")
	...
	level_state.callbacks[#level_state.callbacks+1] = set_callback(function ()
		if frames % 150 == 0 then
			...
			for i = 1,#bubble_xy_2 do
				local bubble_uid = spawn(ENT_TYPE.ACTIVEFLOOR_BUBBLE_PLATFORM, bubble_xy_2[i][1], bubble_xy_2[i][2], 0, 0, 0)
				local bubble_ent = get_entity(bubble_uid)
				if bubble_ent ~= nil then
					bubble_ent:set_post_update_state_machine(function(self)
						bubble_ent.velocityy = -0.04
					end)
				end
			end
		end
